		...
		this.container.getElements('.th').each(function(el,i){
			...
			if(dataType){
				...
				el.getdate = function(str){
					...
					var myRegExp = new RegExp("(\d{2,4})-(\d{1,2})-(\d{1,2})");
					if (ret = str.match(myRegExp)) {
						return (fixYear(ret[1])*10000) + (ret[2]*100) + (+ret[3]) + strtime;
					}
					// DD/MM/YY[YY] or DD-MM-YY[YY]

						myRegExp = new RegExp("(\d{1,2})[\/-](\d{1,2})[\/-](\d{2,4})");
					if (ret = str.match(myRegExp)) {
...
Element.implement({
	getTextWidth:function(){
		...
		this.empty();
		var span = new Element('span').set('text',text).injectInside(this);
