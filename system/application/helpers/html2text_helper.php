	...
	if( is_object($sourceStr) ) {
		...
		if( $isXML ) {
			...
			if( $strippedStr === null ) {
				...
				//trigger_error('String passed to html2text has encoding issues (contains characters not permitted in the encoding PHP is using) - attempting recovery by not stripping PHP from the source string:' . $sourceStr,E_USER_WARNING);
