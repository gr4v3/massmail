...
class mail extends POG_Base
{
	...
	*/
	...
	function mail($subject='', $message='', $state='', $created='', $modified='', $deleted='')
	{
	...
	*/
	function Get($mailId)
	{
		...
		$this->pog_query = "select * from `mail` where `mailid`='".intval($mailId)."' LIMIT 1";
	...
	*/
	function GetList($fcv_array = array(), $sortBy='', $ascending=true, $limit='')
	{
		...
		$this->pog_query = "select * from `mail` ";
		$mailList = Array();
		...
		while ($row = Database::Read($cursor))
		{
			$mail = new $thisObjectName();
			$mail->mailId = $row['mailid'];
			$mail->subject = $this->Unescape($row['subject']);
			$mail->message = $this->Unescape($row['message']);
			$mail->state = $this->Unescape($row['state']);
			$mail->created = $row['created'];
			$mail->modified = $row['modified'];
			$mail->deleted = $row['deleted'];
			$mailList[] = $mail;
		}
		return $mailList;
	}
	...
	* @return integer $mailId
	*/
	function Save()
	{
		$connection = Database::Connect();
		$this->pog_query = "select `mailid` from `mail` where `mailid`='".$this->mailId."' LIMIT 1";
		$rows = Database::Query($this->pog_query, $connection);
		if ($rows > 0)
		{
			$this->pog_query = "update `mail` set 
			`subject`='".$this->Escape($this->subject)."', 
			`message`='".$this->Escape($this->message)."', 
			`state`='".$this->Escape($this->state)."', 
			`created`='".$this->created."', 
			`modified`='".$this->modified."', 
			`deleted`='".$this->deleted."' where `mailid`='".$this->mailId."'";
		}
		else
		{
			$this->pog_query = "insert into `mail` (`subject`, `message`, `state`, `created`, `modified`, `deleted` ) values (
	...
	*/
	...
	{
		$connection = Database::Connect();
		$this->pog_query = "delete from `mail` where `mailid`='".$this->mailId."'";
	...
	*/
	function DeleteList($fcv_array)
	{
		...
		{
			$connection = Database::Connect();
			$pog_query = "delete from `mail` where ";
			for ($i=0, $c=sizeof($fcv_array); $i<$c; $i++)
			{
				if (sizeof($fcv_array[$i]) == 1)
				{
