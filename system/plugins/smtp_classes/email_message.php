 ...
 * email_message.php
 *
 * @(#) $Header: /home/mlemos/cvsroot/mimemessage/email_message.php,v 1.90 2009/08/12 04:23:48 mlemos Exp $
 *
 *
 */
...
<class>
	...
	<version>@(#) $Id: email_message.php,v 1.90 2009/08/12 04:23:48 mlemos Exp $</version>
...
class email_message_class
{
	...
	var $headers=array("To"=>"","Subject"=>"","Message-ID"=>"");
	var $body=-1;
	var $body_parts=0;
	var $parts=array();
	var $total_parts=0;
...
*/
	...
	Function GenerateRawMessageID($sender,$brackets = true)
	{
		...
		if($brackets) return "<".uniqid()."@".preg_replace('/[^.A-Za-z_-]/', '', $host).">";
//		else return strftime("%Y%m%d%H%M%S", $micros)."@".preg_replace('/[^.A-Za-z_-]/', '', $host);
		else return uniqid()."@".preg_replace('/[^.A-Za-z_-]/', '', $host);
	}
	...
	Function GenerateMessageID($sender)
	{
		...
		&& $host[strlen($host)-1]=="-")
			$host=substr($host,0,strlen($host)-1);
//		return($this->FormatHeader("Message-ID", "<".strftime("%Y%m%d%H%M%S", $seconds).substr($micros,1,5).".".preg_replace('/[^A-Za-z]/', '-', $local)."@".preg_replace('/[^.A-Za-z_-]/', '', $host).">"));
		return($this->FormatHeader("Message-ID", "<".uniqid()."@".preg_replace('/[^.A-Za-z_-]/', '', $host).">"));
	}
